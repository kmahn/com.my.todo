...
export class TodoRepositoryImpl implements TodoRepository {
  ...
  async findAll(filter: FilterQuery<Partial<Todo>> = {}): Promise<Partial<Todo[]>> {
  ...
  async findOne(id: string): Promise<Partial<Todo>> {
    const document = await this.todoModel.findById(id).populate('user').lean();
    if (!document) {
      throw new TodoNotFoundException();
    }
    return document;
  ...
  async updateOne(id: string, todo: Partial<Todo>): Promise<void> {
    const document = await this.todoModel.findByIdAndUpdate(id, todo);
    if (!document) {
      throw new TodoNotFoundException();
    }
  }
